...
export function SortableTable({
  ...
  const columns = elements(elements(head?.props.children)[0]?.props.children);
    ...
    <>
      <div className="mobile-table-sort">
          ...
          >
            ...
            {columns.map((column, index) => (
              <option key={index} value={index}>
                {Children.toArray(column.props.children)
                  .filter(
                    (node) => !isValidElement(node) || node.type !== "small",
                  )
                  .map(text)
                  .join(" ")}
              </option>
        ...
        <button
          ...
          onClick={() => sort && select(sort.column)}
        >
