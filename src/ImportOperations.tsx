import { YahooConnection } from "./YahooConnection";
...
export function ImportOperations({ owner }: { owner: boolean }) {
  ...
  return (
    <>
      <YahooConnection owner={owner} />
      <section className="panel">
        <h3>Yahoo refresh</h3>
        <p>
          Connect Yahoo above to enable automatic server sync. Until the first
          successful API sync, the existing importer remains available. Logs
        ...
        <button
          ...
          onClick={async () => {
            ...
            try {
              ...
              setNotice(result.message || "Refresh queued.");
            } catch (e) {
              setNotice(
                e instanceof Error ? e.message : "Unable to queue refresh.",
              );
            } finally {
              setBusy(false);
            }
